...
/**
 * Standalone Dubbo control plane console.
 *
 ...
 * export. See DESIGN.md for the field-by-field mapping.
 */
...
const CONFIG = (() => {
  const el = document.getElementById("dubbod-console-config");
  return el ? JSON.parse(el.textContent) : { basePath: "/gui", product: "dubbod GUI" };
})();

const apiURL = (resource, controlPlane = "all") => {
  const url = new URL(`api/${resource}`, document.baseURI);
  url.searchParams.set("controlPlane", controlPlane || "all");
  return url;
};
...
const fetchLogs = (target, controlPlane) => {
  const url = apiURL("logs", controlPlane);
...
const themeKey = "dubbod-console-theme";
...
const ControlPlanePicker = ({ data, value, onChange }) => {
  const planes = data.controlPlanes || [];
  const clusters = data.clusters || [];
  const healthy = planes.filter((plane) => plane.healthy).length;
  return html`
    <div class="plane-picker">
      <label class="plane-picker-label" for="control-plane">${t("controlPlane.scope")}</label>
      <select id="control-plane" class="plane-picker-select" value=${value}
        onChange=${(event) => onChange(event.target.value)}>
        <option value="all">${t("controlPlane.all", clusters.length, healthy, planes.length)}</option>
        ${planes.map((plane) => html`
          <option key=${plane.id} value=${plane.id}>
            ${plane.healthy ? "●" : "○"} ${plane.name}${plane.clusterId ? ` · ${plane.clusterId}` : ""}
          </option>
        `)}
      </select>
      <div class="plane-picker-meta">
        ${value === "all"
          ? t("controlPlane.summary", clusters.length, planes.length)
          : planes.find((plane) => plane.id === value)?.podName || t("controlPlane.unavailable")}
      </div>
    </div>
  `;
};
      ...
      <aside class="drawer drawer-wide">
        ...
        <div class="drawer-body log-body">
          ...
          ${!state.loading && !state.error && pods.map((p) => {
            ...
            return html`
              <section class="log-pod" key=${`${p.clusterId || ""}/${p.name}/${p.container}`}>
                <div class="log-pod-head">
                  ${p.clusterId && html`<span class="chip">${p.clusterId}</span>`}
...
const byNamespace = (workloads, podTotals = {}) => {
  ...
  for (const w of workloads) {
    const cluster = w.clusterId || "";
    const key = `${cluster}/${w.namespace}`;
    const row = rows.get(key) || {
      cluster, namespace: w.namespace, injected: 0, accepting: 0, restarts: 0,
  ...
  return [...rows.values()]
    .map((r) => ({
      ...
      candidates: Math.max(podTotals[`${r.cluster}/${r.namespace}`] ?? podTotals[r.namespace] ?? r.injected, r.injected),
    }))
    .sort((a, b) => `${a.cluster}/${a.namespace}`.localeCompare(`${b.cluster}/${b.namespace}`));
};
      ...
      <section class="section">
        ...
        ${instances.length > 0 && html`
          <div class="table-wrap">
            <table class="table">
              <thead><tr><th>${t("col.pod")}</th><th>${t("col.cluster")}</th><th>${t("col.podIP")}</th><th>${t("col.namespace")}</th><th>${t("col.state")}</th></tr></thead>
              <tbody>
                ${instances.map((i) => html`
                  <tr key=${`${i.clusterId || ""}/${i.name}`}>
                    <td class="mono cell-strong">${i.name || "–"}</td>
                    <td class="mono cell-muted">${i.clusterId || data.clusterId || "–"}</td>
      ...
      <section class="section">
        ...
        ${namespaces.length > 0 && html`
          <div class="table-wrap">
            <table class="table">
              <thead>
                <tr><th>${t("col.cluster")}</th><th>${t("col.namespace")}</th><th>${t("col.injected")}</th><th>${t("col.inboundL4")}</th><th>${t("col.mtls")}</th><th>${t("col.certificate")}</th><th>${t("col.trustRoot")}</th><th>${t("col.restarts")}</th></tr>
              </thead>
              <tbody>
                ${namespaces.map((ns) => html`
                  <tr key=${`${ns.cluster}/${ns.namespace}`} class="row-click"
                    onClick=${() => openDrawer({ type: "namespace", title: ns.namespace, data: ns })}>
                    <td class="mono cell-muted">${ns.cluster || data.clusterId || "–"}</td>
      ...
      <section class="section">
        ...
        ${gateways.length > 0 && html`
          <div class="table-wrap">
            <table class="table">
              <thead>
                <tr><th>${t("col.deployment")}</th><th>${t("col.cluster")}</th><th>${t("col.gateway")}</th><th>${t("col.namespace")}</th><th>${t("col.class")}</th><th>${t("col.replicas")}</th><th>${t("col.state")}</th></tr>
              </thead>
              <tbody>
                ${gateways.map((g) => {
                  const status = g.isReady ? "ok" : (g.readyReplicas || 0) === 0 ? "err" : "warn";
                  return html`
                    <tr key=${`${g.clusterId || ""}/${g.namespace}/${g.name}`} class="row-click"
                      onClick=${() => openDrawer({ type: "gateway", title: g.name, status, data: g })}>
                      <td class="mono cell-strong">${g.name}</td>
                      <td class="mono cell-muted">${g.clusterId || data.clusterId || "–"}</td>
...
const ServicesPage = ({ data, openDrawer, onRefresh, refreshing }) => {
  ...
  const filtered = useMemo(() => {
    ...
    return services
      ...
      .filter((s) => !q || [s.name, s.hostname, s.namespace, s.clusterId].some((v) => v?.toLowerCase().includes(q)))
          ...
          <table class="table table-wide">
            <thead>
              <tr>
                ...
                <${Th} k="clusterId">${t("col.cluster")}</${Th}>
            ...
            <tbody>
              ${filtered.map((s) => html`
                <tr key=${`${s.clusterId || ""}/${s.hostname}`} class="row-click"
                  ...
                  <td class="mono cell-muted">${s.clusterId || data.clusterId || "–"}</td>
...
const App = () => {
  ...
  const [controlPlane, setControlPlane] = useState(() => {
    try { return localStorage.getItem("dubbod-console-control-plane") || "all"; } catch (_) { return "all"; }
  });
  ...
  const loadOverview = useCallback(async () => {
    try {
      setData(await getJSON(apiURL("overview", controlPlane)));
  ...
  const pollMetrics = useCallback(async () => {
    try {
      setMetrics(await getJSON(apiURL("metrics", controlPlane)));
  ...
  const openLogs = useCallback(async (target, tail = 200) => {
    ...
    setLogsState(next);
    try {
      setLogsState({ ...next, loading: false, data: await fetchLogs({ ...target, tail }, controlPlane) });
    } catch (e) {
      setLogsState({ ...next, loading: false, error: e.message || String(e) });
    }
  }, [controlPlane]);

  const selectControlPlane = useCallback((next) => {
    setControlPlane(next);
    setDrawer(null);
    setLogsState(null);
    try { localStorage.setItem("dubbod-console-control-plane", next); } catch (_) { /* storage unavailable */ }
  }, []);
  ...
  if (!data && overviewError) {
    return html`
      <div class="app is-fault">
        <main class="main main-solo">
          <div class="fault">
            <svg class="fault-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linejoin="round" aria-hidden="true">
              <path d="M12 3.4 21.4 20.2H2.6L12 3.4z" />
              <path d="M12 9.4v5.1" stroke-linecap="round" />
              <circle cx="12" cy="16.9" r="0.75" fill="currentColor" stroke="none" />
            </svg>
            <p class="fault-code">${overviewError}</p>
            <button class="fault-retry" type="button" onClick=${loadOverview}>Retry</button>
          </div>
        </main>
      </div>
    `;
    ...
    <div class="app">
      <aside class="sidebar">
        <div class="brand">
          ...
          <span class="brand-name">${CONFIG.product}</span>
        </div>
        <${ControlPlanePicker} data=${data} value=${controlPlane} onChange=${selectControlPlane} />
        <nav class="nav">
          ${NAV.map((group) => html`
